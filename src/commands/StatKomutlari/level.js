...
run: async (client, message, args, prefix) => {
    if (!message.guild) return;
        ...
        let loading = await message.channel.send({ content: `<a:sivra_xp:1131909595359621130> Seviye Verileri Yükleniyor...`})
        const background = await loadImage("https://cdn.discordapp.com/attachments/853287752296562698/871936289652478092/Astena_krmz.png")
        ...
        message.channel.send({ content: `<a:bit:1143530581771169832> **[** \`${target.user.tag}\` **]** kullanıcısının sunucu level kartı!`, files:  [attachment] });
}
};
