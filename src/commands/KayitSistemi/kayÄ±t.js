...
`)]})
   ...
   const registerData = await register.findOne({ guildID: message.guild.id, userID: uye.user.id });
...
${data ? data.names.splice(0, 3).map((x, i) => `\`${x.name}\` (${x.rol}) (<@${x.yetkili}>) [<t:${Math.floor(x.date / 1000)}:R>]`).join("\n") : "Daha önce kayıt olmamış."}
    `)
...
if(button.customId === "MAN") {
 ...
 let manss = new MessageEmbed()
  ...
  .setDescription(`
  ${uye.toString()} Üyesinin İsmi \`"${setName}"\` Olarak Değiştirildi 
